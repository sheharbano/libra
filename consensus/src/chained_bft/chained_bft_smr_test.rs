...
#[test]
/// Checks that DropConfigRound (message dropping rules per round) works for twins
///
/// Setup:
...
///
/// Run the test:
/// cargo xtest -p consensus twins_drop_config_round_test -- --nocapture
fn twins_drop_config_round_test() {
    ...
    // Specify round leaders here
    // Will default to the first node, if no leader specified for given round
    let twins_round_proposers_idx: HashMap<Round, Vec<usize>> = HashMap::new();
    ...
    let _n2 = nodes[2].smr.author();
    let _n3 = nodes[3].smr.author();
...
fn twins_safety_violation_test() {
    ...
    block_on(async move {
        ...
        // =================
        // Check if the commit logs across the two partitions matches
        // =================
...
///
/// Run the test:
/// cargo xtest -p twins_safety_violation_scenario_executor_test -- --nocapture
...
fn execute_scenario(
    ...
    for i in 0..num_nodes {
        ...
        match &block_store {
           ...
           /*
            _          => {
                debug!("Safety check does not include tree of node {:?} due to NULL block store.", i);
            },
            */
        }
    }
